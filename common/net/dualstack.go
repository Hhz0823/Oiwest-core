...
func (d *DualStackDialer) dialParallel(ctx context.Context, network string, ipv4Addrs, ipv6Addrs []net.IP, port int) (net.Conn, error) {
	...
	resultCh := make(chan result, len(ipv4Addrs)+len(ipv6Addrs))
...
func (d *DualStackDialer) DialWithStrategy(ctx context.Context, network, address string) (net.Conn, error) {
	switch d.config.Strategy {
	case "latency":
		return d.dialLowestLatency(ctx, network, address)
	case "random":
		return d.dialRandom(ctx, network, address)
	case "roundrobin":
		return d.dialRoundRobin(ctx, network, address)
	case "multiline":
		return d.dialMultiLine(ctx, network, address)
	default:
		return d.DialContext(ctx, network, address)
	}
}

func (d *DualStackDialer) dialLowestLatency(ctx context.Context, network, address string) (net.Conn, error) {
	return d.DialContext(ctx, network, address)
}

func (d *DualStackDialer) dialRandom(ctx context.Context, network, address string) (net.Conn, error) {
	...
	addr := fmt.Sprintf("%s:%d", ips[idx].IP.String(), port)
	return (&net.Dialer{}).DialContext(ctx, network, addr)
}

var roundRobinCounter uint64

func (d *DualStackDialer) dialRoundRobin(ctx context.Context, network, address string) (net.Conn, error) {
	host, portStr, _ := net.SplitHostPort(address)
	port := 0
	fmt.Sscanf(portStr, "%d", &port)
	ips, _ := net.DefaultResolver.LookupIPAddr(ctx, host)
	...
	idx := atomic.AddUint64(&roundRobinCounter, 1) % uint64(len(ips))
	addr := fmt.Sprintf("%s:%d", ips[idx].IP.String(), port)
	return (&net.Dialer{}).DialContext(ctx, network, addr)
}

func (d *DualStackDialer) dialMultiLine(ctx context.Context, network, address string) (net.Conn, error) {
	d.mu.RLock()
	hasIPv4 := len(d.ipv4Pool.addresses) > 0
	hasIPv6 := len(d.ipv6Pool.addresses) > 0
	d.mu.RUnlock()
	...
	totalLines := 0
	if hasIPv4 {
		totalLines += len(d.ipv4Pool.addresses)
	}
	if hasIPv6 {
		totalLines += len(d.ipv6Pool.addresses)
	}
	resultCh := make(chan dialResult, totalLines)
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
...
func (d *DualStackDialer) probeAddr(addr MultiLineAddr, pool *MultiLinePool) {
	start := time.Now()
	target := fmt.Sprintf("%s:%d", addr.IP, 80)
