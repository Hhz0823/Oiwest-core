...
const (
	...
	writeBufSize       = FrameHeaderSize + MaxFrameSize // Pre-allocated max frame buffer
)
...
type MuxSession struct {
	...
	wg          sync.WaitGroup
	writeBuf    []byte  // Reusable buffer for WriteFrame to avoid per-frame allocation
	writeMu     sync.Mutex // Separate lock for write serialization
}
...
func NewMuxSession(conn net.Conn, config *MuxConfig) *MuxSession {
	...
	s := &MuxSession{
		...
		ctx:        ctx,
		cancel:     cancel,
		acceptCh:   make(chan *MuxStream, 32), // Increased from 16 for better burst handling
...
func (s *MuxSession) WriteFrame(streamID uint32, frameType FrameType, data []byte) (int, error) {
	dataLen := len(data)
	frameSize := FrameHeaderSize + dataLen
	...
	// Use pre-allocated write buffer for frames that fit
	if frameSize <= len(s.writeBuf) {
		frame := s.writeBuf[:frameSize]
...
func (s *MuxSession) readLoop() {
	...
	// Reusable data buffer that grows as needed
	var dataBuf []byte
	...
	for {
		...
		var data []byte
		if dataLen > 0 {
			// Reuse/grow the data buffer instead of allocating per frame
