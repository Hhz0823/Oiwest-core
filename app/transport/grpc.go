...
type GRPCConn struct {
	...
	readPos  int
	readLen  int
	writeBuf []byte  // Reusable write buffer to avoid per-write allocation
	mu       sync.Mutex
	closed   bool
}
...
func NewGRPCConn(ctx context.Context, addr string, port int, settings *StreamSettings) (*GRPCConn, error) {
	...
	conn := &GRPCConn{
		...
		writeBuf: make([]byte, grpcFrameSize+grpcMaxDataSize), // Pre-allocate combined buffer
	}
...
func (c *GRPCConn) Read(p []byte) (int, error) {
	...
	// Reuse a stack-allocated frame header
	var frameHeader [grpcFrameSize]byte
...
func (c *GRPCConn) Write(p []byte) (int, error) {
	...
	for offset < len(p) {
		...
		// Combine frame header + data into a single write to reduce syscall overhead
		totalSize := grpcFrameSize + chunkSize
		if totalSize <= len(c.writeBuf) {
			binary.BigEndian.PutUint32(c.writeBuf[0:4], uint32(chunkSize))
			c.writeBuf[4] = 0
			copy(c.writeBuf[grpcFrameSize:], p[offset:offset+chunkSize])
			if _, err := c.conn.Write(c.writeBuf[:totalSize]); err != nil {
				return written, err
			}
		} else {
...
func (l *GRPCListener) Accept() (net.Conn, error) {
	...
	conn := &GRPCConn{
		...
		config:   l.config,
		readBuf:  make([]byte, 64*1024),
		writeBuf: make([]byte, grpcFrameSize+grpcMaxDataSize),
	}
	return conn, nil
}
